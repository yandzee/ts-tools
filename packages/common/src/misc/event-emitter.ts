...
export class EventEmitter<T extends Handlers<any>> {
  private onHandlers: Map<keyof T, Set<HandlerFn>>;
  private onceHandlers: Map<keyof T, Set<HandlerFn>>;
  ...
  private emitCached<K extends keyof T>(evt: K, handler: T[K], once?: boolean) {
    ...
    if (once) {
      const datum = cached.shift() ?? ([] as any[]);
      this.onceHandlers.get(evt)?.delete(handler);

      if (datum) handler(...datum);
      return;
    }
